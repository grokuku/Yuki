...
export class MarkdownSpeechFilter {
    ...
    while (i < n) {
      ...
      if (this.inFence) {
        ...
        if (isFenceDelimiter(raw.slice(i, nl), this.fenceChar)) {
          this.inFence = false;
          if (this.codeAnnouncement) out += `${this.codeAnnouncement} `;
        }
        i = nl + 1;
        this.atLineStart = true;
        continue;
  ...
  private readLink(
    ...
    if (inner.includes("\n")) {
      return { text: raw.slice(i, close + 1), next: close + 1 };
    }
    if (raw[close + 1] === "(") {
      const closeParen = raw.indexOf(")", close + 2);
      if (closeParen === -1) {
        if (!final) return null;
        return { text: inner, next: raw.length };
      }
      return { text: inner, next: closeParen + 1 };
    }
    return { text: raw.slice(i, close + 1), next: close + 1 };
  }
}
